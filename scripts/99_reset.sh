...
#
# Deux niveaux (les documents sources data/sources/ ne sont JAMAIS touchés) :
#
...
#
#   ./scripts/99_reset.sh --complet  Tout : supprime aussi data/work/ (manifest,
#                                    chunks, extractions, fiches, embeddings,
#                                    journaux). Rebuild COMPLET, avec ré-appels
#                                    LLM (long) :
#                                        ./scripts/run_all.sh
#
...
for chemin in "$KB_DB" "$KB_WORK" "$KB_RACINE"; do
...
if [[ "$COMPLET" == "1" ]]; then
  ...
  echo "Conservés : $KB_WORK/ (cache de travail) et data/sources/ (documents)."
fi
echo "Jamais touchés : data/sources/ et config/."
